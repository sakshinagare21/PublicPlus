    ...
    return (
        <div className="fixed bottom-6 right-6 z-[9999] font-sans">
            ...
            <button
                ...
                className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-2xl transition-all duration-300 hover:scale-110 active:scale-95 ${isOpen ? "bg-red-500 rotate-90" : "bg-primary shadow-primary/30"
                    }`}
            >
                ...
                <div className="absolute bottom-20 right-0 w-[350px] max-h-[500px] bg-card border border-border rounded-3xl overflow-hidden shadow-2xl flex flex-col animate-in slide-in-from-bottom-4 duration-300">
                    ...
                    >
                        {messages.map((msg, i) => (
                            <div
                                key={i}
                                className={`flex gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300 ${msg.type === "user" ? "flex-row-reverse" : ""
                                    }`}
                            >
                                <div className={`p-2 rounded-xl shrink-0 h-fit ${msg.type === "bot" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
                                    }`}>
                                    {msg.type === "bot" ? <Bot className="w-4 h-4" /> : <User className="w-4 h-4" />}
                                </div>
                                <div className={`max-w-[80%] p-3 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.type === "bot"
                    ...
                    <div className="p-4 border-t border-border bg-background flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex-1 bg-muted/50 rounded-xl px-4 py-2 text-[10px] text-muted-foreground   flex items-center gap-2">
