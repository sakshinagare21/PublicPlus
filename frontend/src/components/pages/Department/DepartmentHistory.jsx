...
const DepartmentHistory = () => {
    ...
    const fetchHistory = async () => {
        try {
            ...
            const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/departments/issues`, {
            ...
            <div className="space-y-10 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                    <div className="space-y-1">
                        <h1 className="text-5xl font-black tracking-tighter text-foreground leading-none uppercase">
                            Mission <span className="text-primary">Archives</span>
                        </h1>
                        <p className="text-xs font-bold text-muted-foreground tracking-[0.3em] uppercase opacity-50">Historical Operation Database</p>
                    </div>

                    <div className="relative group/search w-full md:w-96 font-mono">
                            ...
                            >
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className={`px-4 py-1.5 rounded-full border text-[9px] font-bold tracking-widest uppercase ${getStatusColor(issue.status)}`}>
                                            {issue.status}
                                        </div>
                                    </div>

                                    <h3 className="text-xl font-bold text-foreground tracking-tighter leading-tight group-hover:text-primary transition-colors font-sans capitalize">
                                        {issue.title}
                                    </h3>

                                    <div className="flex items-center gap-3 text-muted-foreground font-mono">
                                        <MapPin size={14} className="text-primary/50" />
                                        <span className="text-[10px] font-bold tracking-widest truncate uppercase opacity-60">
                                            {issue.fullAddress || "Sector Core [GRID]"}
                                        </span>
                                    </div>
                                </div>

                                <div className="pt-8 border-t border-border/50 flex items-center justify-between font-mono">
                                    <div className="space-y-1">
                                        <p className="text-[9px] font-bold text-muted-foreground tracking-widest opacity-40 uppercase">Archived Date</p>
                ...
                {!loading && filteredIssues.length === 0 && (
                    <div className="py-40 text-center space-y-6 bg-card/20 rounded-[3rem] border border-dashed border-border font-mono">
                        <div className="w-20 h-20 bg-muted rounded-full flex items-center justify-center mx-auto mb-4">
                            <History size={40} className="text-muted-foreground opacity-30" />
                        </div>
                        <p className="text-[10px] font-bold text-muted-foreground tracking-[0.5em] uppercase">No Historical Records Detected</p>
                    </div>
                )}
                ...
                {showHistoryModal && selectedIssue && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300 font-sans">
                        ...
                        <div className="bg-card w-full max-w-2xl rounded-[3rem] border border-border shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden relative z-10 animate-in zoom-in-95 duration-500">

                            {/* MODAL HEADER */}
                            <div className="p-10 border-b border-border bg-muted/30">
                                <div className="flex justify-between items-start mb-4">
                                    <span className="font-mono text-xs text-primary/50 font-bold tracking-[0.2em] uppercase">SYSTEM AUDIT: #{(selectedIssue._id.slice(-8)).toUpperCase()}</span>
                                    <div className={`px-4 py-1.5 rounded-full border text-[9px] font-bold tracking-widest uppercase ${getStatusColor(selectedIssue.status)}`}>
                                        {selectedIssue.status}
                                    </div>
                                </div>
                                <h2 className="text-3xl font-black text-foreground tracking-tighter uppercase">{selectedIssue.title}</h2>
                            </div>
                            ...
                            <div className="p-10 max-h-[60vh] overflow-y-auto custom-scrollbar space-y-8">
                                {(!selectedIssue.statusHistory || selectedIssue.statusHistory.length === 0) ? (
                                    <div className="py-10 text-center space-y-4 opacity-50 font-mono">
                                        <Clock size={32} className="mx-auto text-muted-foreground" />
                                        <p className="text-[10px] font-bold text-muted-foreground tracking-widest uppercase">Initial Department entry established</p>
                                    </div>
                                ) : (
                                            ...
                                            <div key={index} className="relative pl-12 group">
                                                ...
                                                <div className="space-y-2">
                                                    <div className="flex items-center gap-3 font-mono">
                                                        <div className={`flex items-center gap-2 px-3 py-1 rounded-lg border text-[9px] font-bold tracking-widest uppercase ${getStatusColor(log.status)}`}>
                                                    ...
                                                    <p className="text-sm font-bold text-foreground leading-relaxed opacity-60 group-hover:opacity-100 transition-opacity">
                                                        "{log.remark || 'Standard operational transition logged by department protocol.'}"
                                                    </p>
                                                </div>
                                            </div>
                                        ))}
                            ...
                            {/* MODAL FOOTER */}
                            <div className="p-10 border-t border-border flex justify-end font-mono">
                                <button
                                    onClick={() => setShowHistoryModal(false)}
                                    className="bg-primary text-primary-foreground px-10 py-4 rounded-2xl font-bold text-[10px] tracking-widest transition-all active:scale-95 shadow-lg shadow-primary/20 hover:opacity-90 uppercase"
                                >
                                    Terminate Session
                                </button>
                            </div>
