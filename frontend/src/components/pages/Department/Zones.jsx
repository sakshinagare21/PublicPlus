...
const Zones = () => {
 ...
 <p className="text-sm text-muted-foreground font-medium">Scanning territories...</p>
 </div>
 ) : (
 ...
 <h3 className="text-sm font-semibold text-muted-foreground tracking-widest">Department Precincts</h3>
 ...
 <span className="text-xs font-medium text-muted-foreground">{data.total} Active Indicators</span>
 </div>
 {isSelected && (
 ...
 {/* PERSONNEL ROSTER */}
 <div className="bg-card border rounded-2xl p-8 shadow-sm flex flex-col h-fit">
 <div className="flex justify-between items-center mb-8">
 <h3 className="text-sm font-semibold text-muted-foreground tracking-widest">Zone Personnel</h3>
 <Users size={16} className="text-muted-foreground" />
 </div>

 <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
 {activeZoneOperators.length > 0 ? (
 activeZoneOperators.map((op) => (
 <div key={op._id} className="p-4 bg-background border rounded-xl hover:border-primary/30 transition-all cursor-default">
 <div className="flex items-center justify-between mb-3">
 <div>
 <p className="font-semibold text-sm">{op.fullName}</p>
 <p className="text-[10px] text-muted-foreground lowercase">{op.email}</p>
 </div>
 <div className={`h-2.5 w-2.5 rounded-full ${op.status === 'active' ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`} />
 </div>
 
 <div className="space-y-2 mt-4">
 <div className="flex justify-between text-[9px] font-black text-muted-foreground">
 <span>Load Vector</span>
 <span>{op.currentActiveTasks} / {op.maxCapacity} Items</span>
 </div>
 <div className="w-full bg-muted h-1 rounded-full overflow-hidden">
 <div 
 className="bg-primary h-full transition-all duration-1000" 
 style={{ width: `${(op.currentActiveTasks / op.maxCapacity) * 100}%` }} 
 />
 </div>
 </div>
 </div>
 ...
 <p className="text-xs font-semibold tracking-widest">Unmanned territory</p>
 </div>
 )}
 </div>

 <button className="w-full mt-8 py-3 bg-muted border border-border rounded-xl text-xs font-bold text-muted-foreground hover:bg-primary hover:text-white transition-all flex items-center justify-center gap-2">
 Manage Assignments
 <ArrowRight size={14} />
 </button>
 </div>
