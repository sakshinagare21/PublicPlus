...
export default function TaskOperations() {
 ...
 axios.get("http://127.0.0.1:5000/api/issues/admin/all", {
 headers: { Authorization: `Bearer ${token}` },
 }),
 axios.get("http://127.0.0.1:5000/api/issues/admin/stats", {
 headers: { Authorization: `Bearer ${token}` },
 }),
 ]);
 ...
 <div
 key={issue._id}
 className={`bg-[hsl(var(--card))] border transition-all rounded-xl p-5 cursor-pointer hover:border-[hsl(var(--primary)/0.5)] ${
 ...
 <div
 key={issue._id}
 className={`bg-[hsl(var(--card))] border transition-all rounded-xl p-5 cursor-pointer hover:border-[hsl(var(--destructive)/0.5)] ${
 selectedIssue?._id === issue._id 
 ...
 <div className={`absolute -left-8 top-0 w-7 h-7 rounded border flex items-center justify-center z-10 ${
 ...
 log.status === "pending" ? "text-[hsl(var(--muted-foreground))]" : "text-[hsl(var(--foreground))]"
 }`}>
 {log.stage}
 </h4>
 <p className="text-[11px] text-[hsl(var(--muted-foreground))]">
