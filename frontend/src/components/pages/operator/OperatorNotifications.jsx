...
const OperatorNotifications = () => {
 ...
 try {
 const res = await axios.get("http://127.0.0.1:5000/api/notification/operator", {
 headers: {
 ...
 try {
 await axios.put("http://127.0.0.1:5000/api/notification/operator/read-all", {}, {
 headers: {
 Authorization: `Bearer ${localStorage.getItem("token")}`,
 },
 });
 ...
 className="flex items-center gap-2 text-[10px] font-black tracking-widest text-primary hover:text-foreground transition-all shadow-glow"
 >
 <Eye size={16} />
 Inspect Source
 </Link>
