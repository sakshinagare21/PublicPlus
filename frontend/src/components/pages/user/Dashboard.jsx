...
const Dashboard = () => {
 ...
 axios.get("http://127.0.0.1:5000/api/issues/stats", config),
 axios.get("http://127.0.0.1:5000/api/issues/all?limit=5", config)
 ]);
 ...
 key={i}
 to={action.to}
 className={`group relative overflow-hidden rounded-3xl p-8 transition-all transform hover:-translate-y-2 shadow-sm ${
 action.primary
 ...
 {item.images?.[0] ? (
 <img src={`http://127.0.0.1:5000${item.images[0].url}`} className="w-full h-full object-cover transition-transform group-hover:scale-110 duration-700" />
 ) : (
