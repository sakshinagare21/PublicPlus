...
const IssueDetail = () => {
 ...
 `http://127.0.0.1:5000/api/issues/${issueId}`,
 {
 headers: {
 ...
 await axios.post(
 `http://127.0.0.1:5000/api/issues/${issueId}/verify`,
 formData,
 { headers: { 
 Authorization: `Bearer ${token}`,
 ...
 try {
 await axios.post(
 `http://127.0.0.1:5000/api/issues/${issueId}/reopen`,
 formData,
 { headers: { 
 Authorization: `Bearer ${token}`,
 "Content-Type": "multipart/form-data" 
 ...
 <span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest border transition-colors ${
 issue.priority?.level === 'critical' ? 'border-destructive text-destructive bg-destructive/10' : 
 ...
 <div className="space-y-4">
 <p className="text-[10px] font-black tracking-[0.2em] text-muted-foreground">Geospatial Evidence Grid</p>
 ...
 <img 
 src={issue.resolution.proof.url.startsWith('http') ? issue.resolution.proof.url : `http://127.0.0.1:5000${issue.resolution.proof.url}`} 
