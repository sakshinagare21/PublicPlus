...
const MapView = () => {
    ...
    const fetchIssues = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/issues/all`, {
                                ...
                                {["CRITICAL", "HIGH", "MEDIUM", "LOW"].map(p => (
                                    ...
                                    >
                                        <span className="flex items-center gap-4">
                                            <div className={`w-2.5 h-2.5 rounded-full shadow-sm ${p === 'CRITICAL' || p === 'HIGH' ? 'bg-destructive' : p === 'MEDIUM' ? 'bg-warning' : 'bg-emerald-500'
                                                }`} />
                            ...
                            <div className="space-y-3 px-1">
                                {categories.map(cat => (
                                    <label key={cat._id} className="flex items-center gap-4 cursor-pointer group">
                                        ...
                                        />
                                        <div className={`w-5 h-5 rounded-lg border flex items-center justify-center transition-all ${filters.categories.includes(cat._id) ? "bg-primary border-primary shadow-lg shadow-primary/20" : "bg-muted/50 border-border group-hover:border-primary/50"
                                            }`}>
                                            {filters.categories.includes(cat._id) && <Plus size={14} className="text-primary-foreground" />}
                                        </div>
                                        <span className={`text-[11px] font-bold transition-colors ${filters.categories.includes(cat._id) ? "text-foreground" : "text-muted-foreground"
                                            }`}>{cat.label}</span>
                                    </label>
                                ))}
                            </div>
                                    ...
                                    >
                                        <div className="w-72 p-1 font-sans">
                                            <div className="flex justify-between items-start mb-4">
                                                <span className={`px-3 py-1 rounded text-[9px] font-black tracking-[0.2em] ${selectedIssue.status === 'resolved' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'
                                                    }`}>
