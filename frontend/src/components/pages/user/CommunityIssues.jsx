...
const CommunityIssues = () => {
 ...
 const res = await axios.get(`http://127.0.0.1:5000/api/issues/all?search=${search}`, {
 ...
 const res = await axios.post(`http://127.0.0.1:5000/api/issues/${id}/${endpoint}`, {}, {
 headers: { Authorization: `Bearer ${token}` }
 });
 ...
 <span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest leading-none shadow-sm transition-colors border ${
 ...
 <img
 src={`http://127.0.0.1:5000${issue.images[0].url}`}
