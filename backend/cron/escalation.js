...
cron.schedule("*/5 * * * *", async () => {
 ...
 const notifyRoles = [
 { targetRole: "admin", message: `SLA Breach: Mission "${issue.title}" has been escalated to Level ${issue.sla.escalationLevel}.` },
 { targetRole: "department", departmentId: issue.assignedDepartment?._id, message: `Action Required: Mission "${issue.title}" breached SLA and is now escalated.` },
 { targetRole: "operator", operatorId: issue.assignedTo?._id, message: `🚨 MISSION ESCALATED: You have breached the deadline for "${issue.title}". Interventon required.` }
 ];
 ...
 issueId: issue._id,
 createdByModel: "Admin" // System triggered
 });
 }
 }
